            ...
            <div class="col-12 col-sm-6 col-xl table--item">
             ...
             <div class="col-12 col-sm-12 col-xl item--text table--item">
                <div class="all--item--action d-flex align-item-center">
                    <div class="all--item--action d-flex align-item-center">
                        <router-link :to="'/assignForm/' + item.id"
                            v-if="item.type === 'user_general' || item.type === 'user_custom' || item.type === 'custom'"
                            class="button--circle is-warning-light"><i class="fas fa-plus"></i></router-link>
                        <router-link :to="'/voucherForm/' + item.id"
    ...
    export default {
        ...
        methods: {
            ...
            async updateFields(id, data) {
                const fields = await repository.updateFields(id, data)
                this.aleartMessage(fields.message)
            },
            async deleteFields(id) {
                const fields = await repository.deleteVoucher(id)
